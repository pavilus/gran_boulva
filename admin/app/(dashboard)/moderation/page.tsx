...
export default async function ModerationPage() {
  ...
  const [{ data: args }, { data: reports }] = await Promise.all([
    supabase
      .from("arguments")
      .select("id, body, status, like_count, dislike_count, reply_count, created_at, matchup_id, user_id, option_id, media_url, media_type, media_duration, matchup:matchups(title_ht), user:users(username, avatar_url)")
