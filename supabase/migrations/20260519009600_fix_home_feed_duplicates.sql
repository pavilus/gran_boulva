-- ─────────────────────────────────────────────────────────────────────────────
-- Fix get_recommended_home_feed returning duplicate matchups.
-- The LEFT JOINs on user_interests and trending_scores can produce multiple
-- rows per matchup if either table has more than one matching row.
-- Use DISTINCT ON (m.id) in the scored CTE to guarantee one row per matchup.
-- ─────────────────────────────────────────────────────────────────────────────
...
begin
  ...
  with scored as (
    -- DISTINCT ON (m.id) ensures each matchup appears only once even if
    -- user_interests or trending_scores has multiple matching rows.
