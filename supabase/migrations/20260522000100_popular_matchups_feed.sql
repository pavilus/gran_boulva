-- ─────────────────────────────────────────────────────────────────────────────
-- Add p_popular mode to get_recommended_home_feed.
--
-- When p_popular = true:
--   • Only matchups with total_votes > 0 are returned (genuinely voted-on)
--   • Sorted by total_votes DESC, engagement_score DESC  (pure popularity)
--   • Personalization / recommendation noise is bypassed
--
-- When p_popular = false (default, unchanged):
--   • All published matchups (including new ones with 0 votes)
--   • Sorted by recommendation_score (interest + trending + freshness + diversity)
-- ─────────────────────────────────────────────────────────────────────────────
...
begin
  ...
  with scored as (
    select distinct on (m.id)
      ...
      greatest(
        0,
        40 - (extract(epoch from (now() - coalesce(m.published_at, m.created_at)))
      ...
      (
        select v.option_id from public.votes v
         where v.user_id = v_user_id and v.matchup_id = m.id
         limit 1
    ...
    left join public.user_interests  ui on ui.user_id = v_user_id
                                       and ui.category_id = m.category_id
    left join public.trending_scores ts on ts.content_type = 'matchup'
                                       and ts.content_id   = m.id
   where m.status = 'published'
     and (p_category_id is null or m.category_id = p_category_id)
     -- popular mode: skip matchups nobody has voted on yet
     and (not p_popular or m.total_votes > 0)
  ),
  limited as (
    select * from scored
     order by
       -- popular mode → pure vote count; recommendation mode → personalized score
       (case when p_popular
             then m.total_votes::numeric
             else recommendation_score
        end) desc,
       engagement_score desc,        -- tiebreaker (useful in both modes)
